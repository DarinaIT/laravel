            ...
            <div id="accordion" class="col-lg-8 faq">
                ...
                <div class="card">
                    <div class="card-header" style="background-color: {{$set->m_c}};" id="heading{{$vfaq->id}}">
                        <span><button class="btn btn-link collapsed" style="border:none;color:#ffc107;" data-toggle="collapse" data-target="#collapse{{$vfaq->id}}" aria-expanded="false" aria-controls="collapse{{$vfaq->id}}">{{$vfaq->question}}</button></span>
                    </div>
                    <div id="collapse{{$vfaq->id}}" class="collapse" aria-labelledby="heading{{$vfaq->id}}" data-parent="#accordion" style="">
                        <div class="card-body">
                            <p style="color:black;">{!! $vfaq->answer!!}</p>
                        </div>
                    </div>
                </div>
                @endforeach
