...
*/
// Claimed Coins
Route::get('/claimedcoins', function () {
    return 'Hello World';
});
...
Route::get('/ipnboompay', [PaymentController::class, 'boompayIPN'])->name('ipn.boompay');
    ...
    Route::post('postEmailVerify', [UserController::class, 'postEmailVerify'])->name('send.email-verify');
        Route::group(['middleware'=>'auth:user'], function() {
            Route::middleware(['Blocked', 'CheckStatus', 'Kyc', 'Tfa'])->group(function () {
                ...
                //ADD deposit Kudan form
                Route::get('deposit_kudan', [UserController::class, 'deposit_kudan'])->name('user.deposit_kudan');



                Route::get('depositBTB', [UserController::class, 'depositBTB'])->name('user.depositBTB');
                // Route::get('/depositBTB', function () {
                //     return view("user.fund.depositBTB");
                // });

                Route::get('gift_voucher', [UserController::class, 'gift_voucher'])->name('user.gift_voucher');
                Route::post('bank_transfer', [UserController::class, 'bank_transfersubmit'])->name('bank_transfersubmit');

                Route::post('deposit_kudan', [UserController::class, 'deposit_kudansubmit'])->name('deposit_kudansubmit');
...
Route::group(['prefix' => 'admin', 'middleware' => 'auth:admin'], function () {
    ...
    Route::post('section6/update', [WebController::class, 'section6'])->name('section6');
